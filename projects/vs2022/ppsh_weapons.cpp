...
class CPPSHAMMO : public CBasePlayerAmmo
{
	...
	{
		Precache();
		SET_MODEL(ENT(pev), "models/w_clip_tokorev.mdl");
		CBasePlayerAmmo::Spawn();
	}
	void Precache(void)
	{
		PRECACHE_MODEL("models/w_9mmARclip_empty.mdl");
		PRECACHE_MODEL("models/w_clip_tokorev.mdl");
		PRECACHE_SOUND("items/9mmclip1.wav");
	}
	BOOL AddAmmo(CBaseEntity* pOther)
	{
		int bResult = (pOther->GiveAmmo(12, "ppsh",100) != -1);
		if (bResult)
		{
...
LINK_ENTITY_TO_CLASS(ammo_ppsh, CPPSHAMMO); //создаём патроны из класса
