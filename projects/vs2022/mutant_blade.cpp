...
void CPitdroneSpike::SpikeTouch(CBaseEntity* pOther)
{
	...
	else
	{
		...
		pOther->TakeDamage(pev, pevOwner, (pev->iuser1 > 0) ? pev->iuser1 : gSkillData.pitdroneDmgWhip, DMG_GENERIC | DMG_NEVERGIB);
		if (RANDOM_LONG(0, 1))
...
{
public:
	short g_sModelIndexMetalGibs; //новый гиб
	void Spawn(void);
	void Precache(void);
