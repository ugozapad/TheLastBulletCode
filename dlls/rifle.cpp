...
//LINK_ENTITY_TO_CLASS(weapon_357, CSniperrifle);
...
//выше код от снайпы, который ты ¬лад писал ранее, копипаст, 
//можешь его раскоментить но визуал ругаетс€ на FFADE_IN.
// акого черта ему надо не пон€то.

void CSniperrifle::SecondaryAttack(void)

{
	if (m_pPlayer->pev->fov != 0) //≈сли дальность видени€ не равна 0 
		m_pPlayer->pev->fov = m_pPlayer->m_iFOV = 0; //устанавливаем дальность видени€ на 0

	else if (m_pPlayer->pev->fov != 30) //≈сли дальность видени€ не равна 30
		m_pPlayer->pev->fov = m_pPlayer->m_iFOV = 10; //устанавливаем дальность видени€ на 30

	pev->nextthink = UTIL_WeaponTimeBase() + 0.1;
	m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 1.0; //когда следующа€ альтернативна€ атака
	EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/sniper_zoom.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 93 + RANDOM_LONG(0, 0xF));
}
