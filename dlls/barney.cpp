...
LINK_ENTITY_TO_CLASS(monster_npc_officerSS, CNazzSS);
...
void CNazzSS::Spawn()
{
	...
	if (pev->model)
		...
		SET_MODEL(ENT(pev), "models/newNpc/NaziOfficerSS.mdl");
	UTIL_SetSize(pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX);
...
// Precache - precaches all resources this monster needs
//=========================================================
void CNazzSS::Precache()
{
	if (pev->model)
		PRECACHE_MODEL((char*)STRING(pev->model)); //LRC 
	else
		PRECACHE_MODEL("models/newNpc/NaziOfficerSS.mdl");
...
void CNazzSS::TalkInit()
{
	...
	m_szGrp[TLK_ANSWER] = "ON_ANSWER";
	m_szGrp[TLK_QUESTION] = "ON_QUESTION";
	m_szGrp[TLK_IDLE] = "ON_IDLE";
	m_szGrp[TLK_STARE] = "ON_STARE";
	m_szGrp[TLK_USE] = "ON_OK";
	m_szGrp[TLK_UNUSE] = "ON_WAIT";
	m_szGrp[TLK_STOP] = "ON_STOP";

	m_szGrp[TLK_NOSHOOT] = "ON_SCARED";
	m_szGrp[TLK_HELLO] = "ON_HELLO";

	m_szGrp[TLK_PLHURT1] = "!ON_CUREA";
	m_szGrp[TLK_PLHURT2] = "!ON_CUREB";
	m_szGrp[TLK_PLHURT3] = "!ON_CUREC";

	m_szGrp[TLK_PHELLO] = NULL;	//"BA_PHELLO";		// UNDONE
	m_szGrp[TLK_PIDLE] = NULL;	//"BA_PIDLE";			// UNDONE
	m_szGrp[TLK_PQUESTION] = "ON_PQUEST";		// UNDONE

	m_szGrp[TLK_SMELL] = "ON_SMELL";

	m_szGrp[TLK_WOUND] = "ON_WOUND";
	m_szGrp[TLK_MORTAL] = "ON_MORTAL";

	// get voice for head - just one barney voice for now
	m_voicePitch = 100;
}

void CNazzSS::AlertSound(void)
{
	if (m_hEnemy != NULL)
	{
		if (FOkToSpeak())
		{
			PlaySentence("ON_ATTACK", RANDOM_FLOAT(2.8, 3.2), VOL_NORM, ATTN_IDLE);
		}
	}

}

int	CNazzSS::Classify(void)
{
	return	CLASS_HUMAN_MILITARY;
}

int CNazzSS::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	// make sure friends talk about it if player hurts talkmonsters...
	int ret = CTalkMonster::TakeDamage(pevInflictor, pevAttacker, flDamage, bitsDamageType);
	if (!IsAlive() || pev->deadflag == DEAD_DYING)
	...
	if (m_MonsterState != MONSTERSTATE_PRONE && (pevAttacker->flags & FL_CLIENT))
	{
		...
		if (m_hEnemy == NULL)
		{
			...
			{
				// Alright, now I'm pissed!
				PlaySentence("ON_MAD", 4, VOL_NORM, ATTN_NORM);
			...
			{
				// Hey, be careful with that
				PlaySentence("ON_SHOT", 4, VOL_NORM, ATTN_NORM);
				Remember(bits_MEMORY_SUSPICIOUS);
			}
		}
		else if (!(m_hEnemy->IsPlayer()) && pev->deadflag == DEAD_NO)
...
//в даном случае я пишу сообщение себе
//код трасеаттаке тут скопирован с солдафана, а конкретно с альфанациста-киберсолдата.
// хитбокс 3 у него все тело, все бронированное,но для злого барни нациста ему нужна не броня головы, а 
// триггер стрельбы по рожи или шапке(ну хитбокс там в одну группу можно не так важно) и получим в итоге дроп шапки.
//Пока хз как это сделать, но вечером 12.07.22 постараюсь сделать
void CNazzSS::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
...
void CNazzSS::Killed(entvars_t* pevAttacker, int iGib)
{
	if (pev->body < BARNEY_BODY_GUNGONE)
	{// drop the gun!
		Vector vecGunPos;
...
// of the proper type.
//=========================================================
Schedule_t* CNazzSS::GetSchedule(void)
{
	if (HasConditions(bits_COND_HEAR_SOUND))
	{
		CSound* pSound;
	...
	if (HasConditions(bits_COND_ENEMY_DEAD) && FOkToSpeak())
	{
		PlaySentence("ON_KILL", 4, VOL_NORM, ATTN_NORM);
	}
...
MONSTERSTATE CNazzSS::GetIdealState(void)
{
	return CTalkMonster::GetIdealState();
}
...
Schedule_t* CNazzSS::GetScheduleOfType(int Type)
{
	Schedule_t* psched;
...
void CNazzSS::DeclineFollowing(void)
{
	PlaySentence("ON_POK", 2, VOL_NORM, ATTN_NORM);
}




//Новый барни,упрощенная вариация, вмесото целлого нпс в новом .срр
...
LINK_ENTITY_TO_CLASS(monster_barney_mad, CBadbarney);
...
void CBadbarney::Spawn()
{
	...
	if (pev->model)
		...
		SET_MODEL(ENT(pev), "models/newNpc/NaziOfficer.mdl");
	UTIL_SetSize(pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX);
...
void CBadbarney::Precache()
{
	if (pev->model)
		...
		PRECACHE_MODEL("models/newNpc/NaziOfficer.mdl");
...
// триггер стрельбы по рожи или шапке(ну хитбокс там в одну группу можно не так важно) и получим в итоге дроп шапки.
//Пока хз как это сделать, но вечером 12.07.22 постараюсь сделать
void CBadbarney::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	if (ptr->iHitgroup == 10 && (bitsDamageType & (DMG_BULLET | DMG_SLASH | DMG_CLUB)))
	{
		// hit armor
...
void CBadbarney::Killed(entvars_t* pevAttacker, int iGib)
	{
		if (pev->body < BARNEY_BODY_GUNGONE)
		{// drop the gun!
			Vector vecGunPos;
...
void CBadbarney::DeclineFollowing(void)
{
	PlaySentence("ON_POK", 2, VOL_NORM, ATTN_NORM);
}
...
/// Dead mad Barney
/// </summary>
class CNazzSSDead : public CBaseMonster
{
public:
	void Spawn(void);
...
char* CNazzSSDead::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };

void CNazzSSDead::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "pose"))
	{
		m_iPose = atoi(pkvd->szValue);
		...
		CNazzSSDead::KeyValue(pkvd);
}

LINK_ENTITY_TO_CLASS(monster_nazzSS_dead, CNazzSSDead);

//=========================================================
// ********** DeadBarney SPAWN **********
//=========================================================
void CNazzSSDead::Spawn()
{
	if (pev->model)
	{
...
/// Dead mad Barney
/// </summary>
class CDeadBarneyMad : public CBaseMonster
{
public:
	void Spawn(void);
	int	Classify(void) { return	CLASS_HUMAN_MILITARY; }
...
char* CDeadBarneyMad::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };

void CDeadBarneyMad::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "pose"))
	{
		m_iPose = atoi(pkvd->szValue);
...
LINK_ENTITY_TO_CLASS(monster_barney_dead_mad, CDeadBarneyMad);
...
/// <summary>
/// Не рабочий труп толстяка
/// </summary>
// class CDeadFat : public CBaseMonster
//{
//public:
//	void Spawn(void);
//	int	Classify(void) { return	CLASS_HUMAN_MILITARY; }
...
//};
//
//char* CDeadFat::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };
//
//void CDeadFat::KeyValue(KeyValueData* pkvd)
//{
//	if (FStrEq(pkvd->szKeyName, "pose"))
//	{
//		m_iPose = atoi(pkvd->szValue);
...
//}
//
//LINK_ENTITY_TO_CLASS(monster_fat_dead, CDeadFat);
//
////=========================================================
//// ********** DeadFatMen SPAWN **********
////=========================================================
...
//	else
//	{
//		PRECACHE_MODEL("models/newNpc/Hans_Land.mdl");
//		SET_MODEL(ENT(pev), "models/newNpc/Hans_Land.mdl");
//	}
//
//	pev->effects = 0;
//	pev->yaw_speed = 8;
