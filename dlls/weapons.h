...
#define AMMO_VENOMCLIP_GIVE	 VENOM_MAX_CLIP	
...
class CVenom : public CBasePlayerWeapon
{
public:
	void Spawn(void);
	void Precache(void);
...
{
public: //ќбъ€вл€ем все действи€, которые нож должен выполн€ть. Ёти действи€ расположены в файле нашего ножа knife.cpp
	void Spawn(void);
	void Precache(void);
	int iItemSlot(void) { return 1; }
	void EXPORT SwingAgain(void);
...
public:
	void Spawn(void);
	void Precache(void);
	BOOL Deploy(void);//наша функци€
	void WeaponIdle(void);
	void PrimaryAttack();
	void Reload(void); //декларируем перезар€дку
	int iItemSlot(void) { return 0; }
	int GetItemInfo(ItemInfo *p);
	int AddToPlayer(CBasePlayer*);
